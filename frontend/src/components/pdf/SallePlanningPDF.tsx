...
export interface SallePlanningRow {
...
export interface SallePlanningPDFProps {
  salleNom: string
  poleNom?: string | null
  planning: SallePlanningRow[]
...
const MAX_CRENEAUX = 24 // 6 jours × 4 sous-créneaux FP
...
const s = StyleSheet.create({
  ...
  docTitle: { fontSize: 12, fontFamily: 'Helvetica-Bold', color: '#FFFFFF', textAlign: 'center', letterSpacing: 0.8 },
  ...
  cell: { flex: 1, minHeight: 52, marginLeft: 3, borderRadius: 4, padding: 5, borderWidth: 1 },
  ...
  cellSub: { fontSize: 7.5, fontFamily: 'Helvetica-Oblique', marginTop: 2 },
  cellEmpty: { flex: 1, minHeight: 52, marginLeft: 3, borderRadius: 4, backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0' },
  cellRepos: { flex: 1, minHeight: 52, marginLeft: 3, borderRadius: 4, backgroundColor: SLATE_LIGHT, borderWidth: 1, borderColor: BORDER, alignItems: 'center', justifyContent: 'center' },
...
function getRow(planning: SallePlanningRow[], jour: JourSemaine, statut: StatutFixe) {
  return planning.find(r => r.jour_semaine === jour && r.statut === statut)
}

function hasAnySession(planning: SallePlanningRow[], jour: JourSemaine) {
...
export function SallePlanningPDF({
  salleNom, poleNom, planning, totalCreneaux, dateGeneration, logoUrl,
}: SallePlanningPDFProps) {
  const joursActifs = JOURS.filter(j => hasAnySession(planning, j)).length
  const tauxOccupation = Math.round((totalCreneaux / MAX_CRENEAUX) * 100)
  const refDoc = `EDT-SALLE-${dateGeneration.replace(/[^0-9]/g, '')}-${salleNom.replace(/\s+/g, '')}`
      ...
      <Page size="A4" orientation="landscape" style={s.page}>
        ...
        <View style={s.headerBand}>
          ...
          <View style={s.titleBlock}>
            <Text style={s.docTitle}>EMPLOI DU TEMPS — SALLE</Text>
        ...
        <View style={s.infoStrip}>
          <View style={s.infoCell}>
            <Text style={s.infoLabel}>Salle</Text>
            <Text style={s.infoValue}>{salleNom}</Text>
          </View>
          {poleNom && (
            <View style={s.infoCell}>
          ...
          <View style={s.infoCell}>
            <Text style={s.infoLabel}>Taux d'occupation</Text>
            <Text style={s.infoValue}>{tauxOccupation}%</Text>
            <Text style={s.infoValueSub}>{totalCreneaux}/{MAX_CRENEAUX} créneaux</Text>
          </View>
        ...
        <View style={s.tableWrap}>
                ...
                if (row) {
                  return (
                    <View key={jour} style={[s.cell, { backgroundColor: slot.bg, borderColor: slot.border }]}>
                      ...
                      {row.groupe_nom && (
                        <Text style={[s.cellSub, { color: slot.fg + 'CC' }]}>{row.groupe_nom}</Text>
                      )}
                    </View>
                  )
                }
          ...
          {planning.length === 0 && (
            <View style={s.emptyMsg}>
              <Text style={s.emptyText}>Aucun créneau planifié pour cette salle</Text>
            </View>
          )}
        </View>
        ...
        <View style={s.summaryBar}>
          <View style={[s.summaryCell, { backgroundColor: BLUE_LIGHT }]}>
            <Text style={[s.summaryBigNum, { color: BLUE }]}>{totalCreneaux}<Text style={[s.summaryNumUnit, { color: BLUE }]}>/{MAX_CRENEAUX}</Text></Text>
            <Text style={s.summaryCaption}>Créneaux occupés</Text>
          </View>
          <View style={[s.summaryCell, { backgroundColor: tauxOccupation === 100 ? '#F0FDF4' : '#FFFBEB' }]}>
            <Text style={[s.summaryBigNum, { color: tauxOccupation === 100 ? GREEN : GOLD }]}>{tauxOccupation}<Text style={[s.summaryNumUnit, { color: tauxOccupation === 100 ? GREEN : GOLD }]}>%</Text></Text>
            <Text style={s.summaryCaption}>Taux d'occupation</Text>
          </View>
          <View style={[s.summaryCell, { backgroundColor: '#FFFBEB' }]}>
            ...
            <Text style={s.summaryCaption}>Jours actifs</Text>
          </View>
          <View style={[s.summaryCellLast, { flex: 3, alignItems: 'flex-start', paddingHorizontal: 16 }]}>
            <Text style={[s.summaryCaption, { marginBottom: 5 }]}>Légende des créneaux FP</Text>
            <View style={s.legendWrap}>
              {[
                { label: 'Présentiel Matin S1 (08h30–11h00)', color: '#3B82F6' },
                { label: 'Présentiel Matin S2 (11h00–13h30)', color: '#2563EB' },
                { label: 'Présentiel Après-midi S1 (13h30–16h00)', color: '#F97316' },
                { label: 'Présentiel Après-midi S2 (16h00–18h30)', color: '#F59E0B' },
              ].map(l => (
        ...
        <View style={s.sigSection}>
          <View style={s.sigBox}>
            <Text style={s.sigTitle}>Responsable Pédagogique</Text>
        ...
        <View style={s.footer}>
          <View style={s.footerLeft}>
            ...
            <Text style={s.footerText}>OFPPT — Salle : {salleNom}</Text>
          </View>
