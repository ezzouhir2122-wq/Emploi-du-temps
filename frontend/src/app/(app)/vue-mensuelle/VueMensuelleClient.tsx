...
import { CalendarRange, ChevronLeft, ChevronRight, FileDown, Loader2, Building2, Users, LayoutDashboard, BookOpen, Layers } from 'lucide-react'
...
export function VueMensuelleClient({
  ...
  // Génère le PDF et l'ouvre dans un nouvel onglet pour prévisualisation
  async function openPreview(key: string, componentName: string, _label: string, filename: string) {
    ...
    try {
      ...
      const C = (mod as any)[componentName]
      const blob = await pdf(createElement(C as any, buildPDFProps()) as any).toBlob()
      const url = URL.createObjectURL(blob)
      const win = window.open(url, '_blank')
      if (!win) {
        // Popup bloqué — télécharger à la place
            ...
            return (
              ...
              >
                ...
                <div className="text-left">
                  <div className="leading-tight">{isLoading ? 'Génération…' : view.label}</div>
                  <div className="text-[9px] opacity-75 font-normal leading-tight mt-0.5 hidden sm:block">
                    {isLoading ? 'PDF en cours, patientez…' : view.desc}
                  </div>
                </div>
              </button>
            )
